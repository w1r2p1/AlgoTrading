...
class BackTesting:
	...
	def prepare_df(self, quote:str, pair:str):
		...
		df_hrs_.columns = ['time', pair+'_open_h', pair+'_high_h', pair+'_low_h', pair+'_close_h']
		df_min_.columns = ['time', pair+'_close_m']
		# Set indexes
	...
	def backtest(self, quote:str, pair:str, starting_balances:dict, indic:str, indic_length:int, alloc_pct:int, stop_loss_pct, plot:bool=False):
		...
		def find_high_of_last_red_candle(df, i_:int):
			...
			is_red = False
			while not is_red:
				if df.loc[df.index[i_], pair+'_close_h'] < df.loc[df.index[i_], pair+'_open_h']:
					is_red = True
				else:
					i_-=1
			return df.loc[df.index[i_], pair+'_high_h']

		def find_low_of_last_green_candle(df, i_:int):
			...
			while not is_green:
				if df.loc[df.index[i_], pair+'_close_h'] > df.loc[df.index[i_], pair+'_open_h']:
					...
					i_-=1
			return df.loc[df.index[i_], pair+'_low_h']
		...
		hourly_close = self.df.loc[:,pair+'_close_h']
		for i in tqdm(range(1, len(hourly_close)-1)):      # tqdm : progress bar

			price = self.df[pair+'_close_h'].iloc[i]
			high_of_last_red_candle  = find_high_of_last_red_candle(df=self.df, i_=i-1)
			low_of_last_green_candle = find_low_of_last_green_candle(df=self.df, i_=i-1)
			# indic_value = self.df[indic_name].iloc[i]
			self.df.loc[self.df.index[i],'signal'] = 'sell' if price > high_of_last_red_candle else 'buy' if price < low_of_last_green_candle else ''


			if status=='just bought':
				# Sell either by signal or stop-loss
				price_now    = Decimal(self.df[pair+'_close_h'].iloc[i])
				...
				if (self.df['signal'].iloc[i]=='sell' and increased_more_than_fees) or stop_loss_trigger:
					...
					price_base_next_minute = Decimal(self.df[pair+'_close_m'].iloc[i+1])
				...
				price_base_this_minute = Decimal(self.df[pair+'_close_h'].iloc[i])
				price_base_next_minute = Decimal(self.df[pair+'_close_m'].iloc[i+1])
